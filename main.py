...
#PlaywrightとBrowserのインスタンスをアプリケーション全体で共有
...
async def lifespan(app: FastAPI):
	"""
	アプリケーション起動時にPlaywrightとブラウザを一度だけ初期化し、
	終了時に安全に閉じることでパフォーマンスを向上させます。
	"""
	global playwright, browser
	print("アプリケーションを起動します...")
	playwright = await async_playwright().start()
	browser = await playwright.firefox.launch(headless=True)
	yield
	print("アプリケーションを終了します...")
	await browser.close()
